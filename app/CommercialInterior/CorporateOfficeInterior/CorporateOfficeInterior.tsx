...
const CorporateOfficeInterior = (): JSX.Element => {
    ...
    const fetchService = async () => {
      try {
        const res = await fetch(
          "https://interiorvillabd.com/api/services/6?depth=1&draft=false&locale=undefined"
        );
  ...
  return (
    <main className="flex flex-col w-full items-start relative bg-white overflow-x-hidden min-h-screen">
      ...
      <SEO
        ...
        image={heroImage}
        url="https://interiorvillabd.com/services/commercial-interior/corporate-and-office-interior-design"
        type="service"
      ...
      <PageHero
        title={hero?.title || "Corporate & Office Interior Design"}        
        bgImage={heroImage}
        breadcrumbs={[
          { label: "Home", href: "/" },
          { label: "Services" },
          { label: "Commercial", href: "/services/commercial-interior" },
          { label: hero?.title || "Corporate & Office Interior Design", isActive: true },
        ]}
      />
